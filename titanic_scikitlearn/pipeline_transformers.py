...
class PandasSelector(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, dtype=None, columns=None, inverse=False, return_vector=True):
        self.dtype = dtype
    ...
    def _check_if_all_columns_present(self, x):
        if not self.inverse and self.columns is not None:
            ...
            if len(missing_columns) > 0:
                ...
                raise KeyError('Keys are missing in the record: %s' % missing_columns_)
...
class PandasFillNas(BaseEstimator, TransformerMixin):
    ...
    def transform(self, x):
        return x.fillna(self.replacement_value)
